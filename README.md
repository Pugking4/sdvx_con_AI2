...
> can be used for demonstration, not for actually playing
...
4. Place all the buttons on the bb in a line and then attach the 2nd Arduino GND pin to the upper GND bb rail.
 ...
 * 2 Encoders + 8 Buttons + 11 HID controllable LED
 * release page
...
I only built it to see if it could actually be used to play USC as I thought the drivers could be a bit iffy but they did in fact work but I had too limited of components to create an actual full guide to making a controller.
...
8. Now organise the components according to the diagram below, make sure to pay attention to the reference number (eg; J7) when placing them.
![[Pasted image 20240323121745.png]](https://github.com/Pugking4/sdvx_con_AI2/blob/main/Pasted%20image%2020240323121745.png)

9. Select the "Add vias" tool from the right toolbar and place vias in front of any pin that says GND, they should be in front, not on the pin, then connect all the GND pins to the vias and for the 01x06 connect both the gnd pins to each resistor and then to the via as seen in the diagram below.
...
Congratulations, you've created an SDVX PCB, you can view it in the 3d viewer to get a better look at it.
...
and with the current way I was going to implement it which was to move the scanner over a strip of colours until a black strip was reached would be very impractical. Then I thought of some hobbies I was passionate about and I thought of making a rhythm game controller, there were quite a few to choose from like an IIDX controller, Chunithm controller and if I wanted an extremely hard challenge, a maimai controller or Ongeki controller, but I ended up choosing one of the more simple ones as this was my first foray into designing a rhythm controller from scratch.
...
I realised after I looked at the PCB I needed to completely restart the design process, at the same time I started researching copper pours which were zones filled with copper to act as a common ground plane or a power plane, it took quite a while to figure out how to do it but I finally figured it out and that I had to make nets in the schematic and then add a filled zone in the PCB editor. My next step was to recreate the schematic again so I built it and made sure to keep looking at the microcontroller pin diagram to make sure all the components were lined up in the PCB and not the schematic.
...
I also had quite a few problems when initially creating the controller IRL using the Leonardo as I could only source tiny pushbuttons which had an abnormally high weight spring in them from Jaycar, and also only had a very low-resolution rotary encoder to use (15 PPR vs 600 PPR which is recommended).  Finding firmware that worked too was extremely hard, first I tried making it myself but couldn't get it to work, I then attempted to find existing firmware but multiple codebases didn't work but I finally found one which worked by modifying a few lines. The 3D printing process was also extremely slow as I had to print, check if it fits and then reprint if it didn't, I probably have around half a kilo of 3d printed SDVX cases and knobs lying around my house. While I was creating the controller too I decided to go in blind as I knew the general workings of the controller but that turned into a dumb idea as it led me down a bunch of small problems which took me forever to solve.
...
Unfortunately, my video/recipe wasn't ready by the time the peer review came around. If I commented on it myself though, I'd say I picked too complex of a topic as the raw video was around 40 min long and I had to speed up the entire video and cut out lulling parts to make it fit within the time but even then I only got it to around 14 min, if I got another chance I'd either choose a simpler topic or make the tutorial more succinct.
